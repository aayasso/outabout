...
class _ThemeOverrideSelector extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final currentOverride =
        ref.watch(userThemeOverrideProvider);

    final options = <({String label, WeatherTheme? theme})>[
      (label: 'Adaptive', theme: null),
      ...WeatherTheme.values.map(
        (t) => (label: t.displayName, theme: t),
      ),
    ];
    ...
    return Wrap(
      ...
      children: options.map((option) {
        final isActive =
            option.theme == currentOverride;

        return GestureDetector(
          onTap: () {
            ref
                .read(
                  userThemeOverrideProvider.notifier,
                )
          ...
          child: Semantics(
            ...
            child: Container(
              ...
              child: Text(
                option.label,
                style: OutAboutTypography.labelSmall(
                  colors,
                ).copyWith(
                  color: isActive
